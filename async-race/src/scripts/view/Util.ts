...
export const getPageNavBox = () => {
  const pageNavBox = document.createElement('div');
  pageNavBox.classList.add('main-box__page-nav-box');
  const pageNavPrev = document.createElement('button');
  ...
  pageNavNext.innerText = 'next';

  pageNavBox.append(pageNavPrev, pageNavNext);
  return pageNavBox;
};
